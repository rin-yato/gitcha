// @ts-nocheck
import type { MouseEvent, Renderable, RenderContext, RGBA } from "@opentui/core";
import {
  BoxRenderable,
  type ColorInput,
  DiffRenderable,
...
import {
  ...
  DiffLineRenderable,
  type DiffLineRenderableOptions,
} from "./diff-line";
...
declare module "@opentui/react" {
  interface OpenTUIComponents {
    "slottable-diff": typeof SlottableDiffRenderable;
  }
}
...
export class SlottableDiffRenderable extends DiffRenderable {
  ...
  private _onScrollStateChange?: (state: {
    scrollTop: number;
    viewportHeight: number;
    scrollHeight: number;
  }) => void;
  ...
  constructor(ctx: RenderContext, options: SlottableDiffOptions) {
    ...
    this.onMouseScroll = this.handleMouseScroll.bind(this);
    this.onMouseDown = this.handleMouseDown.bind(this);
    this.onMouseDrag = this.handleMouseDrag.bind(this);
    this.onMouseDragEnd = this.handleMouseDragEnd.bind(this);
  ...
  private updateVirtualRange(force: boolean = false): void {
    ...
    if (!force && scrollTop === this._scrollTop && viewportHeight === this._viewportHeight)
      return;
  ...
  private renderRange(prevStart: number, prevEnd: number, start: number, end: number): void {
    ...
    if (!this._scrollDragActive && start > prevStart) {
    ...
    if (!this._scrollDragActive && end < prevEnd) {
  ...
  private handleMouseScroll(event: MouseEvent): void {
    ...
    if (delta === 0) return;
    this.scrollBy(delta * (event.scroll?.direction === "up" ? -1 : 1));
  }
  ...
  private handleMouseDrag(event: MouseEvent): void {
    ...
    if (event.y <= threshold) {
      this.scrollBy(-1);
    } else if (event.y >= Math.max(0, this._viewportHeight - 1 - threshold)) {
      this.scrollBy(1);
    }
  }

  private handleMouseDragEnd(): void {
  ...
  public scrollTo(position: number): void {
    const maxScroll = Math.max(0, this.getContentHeight() - this._viewportHeight);
    const next = Math.max(0, Math.min(maxScroll, position));
    if (next === this._scrollTop) return;

    this._scrollTop = next;
    this.translateY = -next;
    if (this._virtualize) {
      this.updateVirtualRange(true);
    }
    this.emitScrollStateChange();
  ...
  private getContentHeight(): number {
    this.ensureOffsets();
    return this._offsets[this.lineCount()] ?? 0;
  }

  private emitScrollStateChange(): void {
  ...
  private getViewportHeight(): number {
    return this._viewportHeight || this.height;
  }
  ...
  public lineCount(): number {
    return this._view === "unified"
      ? this._parsedLines.length
      : Math.ceil(this._parsedLines.length / 2);
  }
...
extend({ "slottable-diff": SlottableDiffRenderable });
