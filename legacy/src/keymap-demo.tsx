...
import type { ActiveKey, Command } from "@opentui/keymap";
...
function getExPromptCommandFieldText(
...
function KeyLabel(props: { children: JSX.Element }) {
  return (
    <span style={{ fg: palette.key, attributes: TextAttributes.BOLD }}>{props.children}</span>
  );
}
...
function CounterPanel(props: {
  ...
  useBindings(() => ({
    ...
    bindings: [
      ...
      {
        key: "return",
        cmd: `:w ${props.saveTarget}`,
        desc: `Write ${props.label.toLowerCase()} panel`,
      },
    ],
  }));
  ...
  return (
    ...
    >
      <text height={1}>
        ...
        <span style={{ fg: props.color, attributes: TextAttributes.BOLD }}>
          {String(props.count())}
        </span>
      </text>
      <text height={1}>
...
function KeymapDemoContent() {
  ...
  const [commandPromptTarget, setCommandPromptTarget] = createSignal<
  ...
  const activeKeys = useKeymapSelector((keymap) =>
  ...
  const getFocusableTargets = (): Renderable[] => {
    return [alphaPanelRef, betaPanelRef, ...editorRefs].filter(
      (target): target is Renderable => target !== undefined,
    );
  };
  ...
  const getFocusableLabel = (target: Renderable): string => {
    ...
    const editorIndex = editorRefs.indexOf(target);
    if (editorIndex !== -1) {
  ...
  const moveFocus = (direction: 1 | -1) => {
    ...
    const currentIndex = targets.indexOf(renderer.currentFocusedRenderable);
  ...
  const selectedCommandPromptSuggestion = createMemo(() => {
    return getSelectedExPromptSuggestion(
  ...
  const executeCommandPrompt = () => {
    ...
    const result = manager.dispatchCommand(parsed.raw, {
  ...
  const focusedEditorIndex = createMemo(() => {
    statusVersion();
    return editorRefs.indexOf(renderer.currentFocusedEditor);
  });
      ...
      >
        <box id="keymap-demo-details-column" flexGrow={1} minWidth={0} flexDirection="column">
          <text id="keymap-demo-status-focused" fg={palette.text} height={1}>
            ...
            <span style={{ fg: focusedColor(), attributes: TextAttributes.BOLD }}>
              {focusedLabel()}
            </span>
          </text>
          ...
          >
            <text fg={palette.text} height={1}>
              ...
              <span style={{ fg: palette.key, attributes: TextAttributes.BOLD }}>
                shift+tab
              </span>
            ...
            <text fg={palette.text} height={1}>
              ...
              <span style={{ fg: palette.textDim }}>
                {" "}
                for line, buffer, and end navigation.
              </span>
            </text>
          </box>
          ...
          <box id="keymap-demo-log" flexDirection="column" marginTop={1}>
            ...
            <Show
              when={logs().length > 0}
              fallback={<text fg={palette.textMuted}>(no events yet)</text>}
            >
        ...
        >
          <text id="keymap-demo-wk-header-text" fg={palette.text} height={1}>
            <span style={{ fg: palette.accent, attributes: TextAttributes.BOLD }}>
              Which Key
            </span>
          ...
          >
            <Show
              when={whichKeyEntries().length > 0}
              fallback={<text fg={palette.textMuted}>(no active keys)</text>}
            >
          ...
          >
            <For each={commandPromptSuggestions()}>
              {(suggestion, index) => {
                const isSelected = () =>
                  index() ===
                  Math.min(commandPromptSelection(), commandPromptSuggestions().length - 1);
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      style={{
                        fg: isSelected() ? palette.title : palette.command,
