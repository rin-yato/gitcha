...
export async function copySelection(renderer: CliRenderer, toast: ToastState): Promise<void> {
  const selectedText = renderer.getSelection()?.getSelectedText();
  if (!selectedText) return;

  return copyToClipboard(selectedText, renderer.copyToClipboardOSC52)
